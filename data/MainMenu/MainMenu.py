...
class Menu:
    ...
    def draw(self, surface):
        raise NotImplementedError(
            'draw function not implemented for this menu {}', self)
...
class LevelSelectorMenu(Menu):
    def __init__(self, main_menu):
        ...
        self.category_selector_ui.add(
            UI.Button(
                self.main_menu.game.DESING_W /
                2 -
                450,
                self.main_menu.game.DESING_H /
                2 -
                200,
                400,
                400,
        ...
        self.search_bar = UI.InputField(
            10, 10, 400, 60, self.update_levels, 'search_icon.png')
    ...
    def load_level_list(self, folder):
        ...
        maps_paths = [os.path.join(folder, map_name)
    ...
    def load_replay(self, btn, args):
        ...
        filepath = filedialog.askopenfilename(
            initialdir='.', title='Select replay to load', filetypes=[
                ('JSON', '.json')])
        if filepath == '':
            return
...
class OptionMenu(Menu):
    ...
    def __init__(self, main_menu):
        super().__init__(main_menu)
        self.fullscreen_toggle = UI.Toggle(
            20, 20, 400, 64, "Fullscreen", self.toggle_fullscreen)
        self.ui_manager.add(self.fullscreen_toggle)

        self.resolution_dropdown = UI.DropDown(20,
                                               100,
                                               400,
                                               60,
                                               ["1920x1080",
        ...
        self.fps_counter_toggle = UI.Toggle(
            20, 180, 400, 64, "FPS Counter", self.toggle_fps)
        self.ui_manager.add(self.fps_counter_toggle)

        self.ui_manager.add(
            UI.Button(
                main_menu.game.DESING_W -
                250,
                main_menu.game.DESING_H -
                100,
                200,
                60,
                "Back",
        ...
        keybind_grid.add(UI.Button, [self.key_to_char(
            cfg['p1_left']), (200, 200, 200), self.set_keybind, ['p1_left'], True])
        keybind_grid.add(UI.Button, [self.key_to_char(
            cfg["p2_left"]), (200, 200, 200), self.set_keybind, ['p2_left'], True])
        keybind_grid.add(UI.Button, [self.key_to_char(
            cfg["p1_right"]), (200, 200, 200), self.set_keybind, ['p1_right'], True])
        keybind_grid.add(UI.Button, [self.key_to_char(
            cfg["p2_right"]), (200, 200, 200), self.set_keybind, ['p2_right'], True])
        keybind_grid.add(UI.Button, [self.key_to_char(
            cfg["p1_jump"]), (200, 200, 200), self.set_keybind, ['p1_jump'], True])
        keybind_grid.add(UI.Button, [self.key_to_char(
            cfg["p2_jump"]), (200, 200, 200), self.set_keybind, ['p2_jump'], True])
        ...
        editor_grid.add(UI.Button, [self.key_to_char(
            cfg['ed_clear']), (200, 200, 200), self.set_keybind, ['ed_clear'], True])
        editor_grid.add(UI.Text, ["Reload", 56, (255, 255, 255)])
        editor_grid.add(UI.Button, [self.key_to_char(
            cfg['ed_reload']), (200, 200, 200), self.set_keybind, ['ed_reload'], True])
        ...
        editor_grid.add(UI.Button, [self.key_to_char(
            cfg['ed_delete']), (200, 200, 200), self.set_keybind, ['ed_delete'], True])
        editor_grid.add(UI.Text, ["Wall", 46, (255, 255, 255)])
        editor_grid.add(UI.Button, [self.key_to_char(
            cfg['ed_wall']), (200, 200, 200), self.set_keybind, ['ed_wall'], True])
        editor_grid.add(UI.Text, ["Door", 46, (255, 255, 255)])
        editor_grid.add(UI.Button, [self.key_to_char(
            cfg['ed_door']), (200, 200, 200), self.set_keybind, ['ed_door'], True])
        editor_grid.add(UI.Text, ["Spawn", 46, (255, 255, 255)])
        editor_grid.add(UI.Button, [self.key_to_char(
            cfg['ed_spawn']), (200, 200, 200), self.set_keybind, ['ed_spawn'], True])
        editor_grid.add(UI.Text, ["Plate", 46, (255, 255, 255)])
        editor_grid.add(UI.Button, [self.key_to_char(
            cfg['ed_plate']), (200, 200, 200), self.set_keybind, ['ed_plate'], True])
        editor_grid.add(UI.Text, ["Goal", 46, (255, 255, 255)])
        editor_grid.add(UI.Button, [self.key_to_char(
            cfg['ed_goal']), (200, 200, 200), self.set_keybind, ['ed_goal'], True])
    ...
    def toggle_fullscreen(self, active):
        ...
        if active:
            ...
            self.main_menu.game.display = pygame.display.set_mode(
                (w, h), FULLSCREEN, HWSURFACE)
        else:
            w, h = self.main_menu.game.w, self.main_menu.game.h
            self.main_menu.game.display = pygame.display.set_mode(
                (w, h), HWSURFACE)
