    ...
    def update(self, mouse_position, mouse_pressed, mouse_rel, events):
        for i, el in enumerate(self.sprites()):
            if el.is_hovered(mouse_position):
                self.sprites()[self.selected].update(
...
class Slider(UIElement):
    def __init__(
        ...
        if self.min_value > self.max_value:
            raise ValueError(
                f"Min value can't be superior to max value : {self}")

    def draw(self):
        self.image.fill(self.bg_color)
        pygame.draw.rect(self.image, self.fg_color,
                         (0, 0, self.draw_w, self.h))
    ...
    def update(self, mouse_position, mouse_pressed, mouse_rel, events):
        if mouse_pressed[0]:
            ...
            if self.callback:
                self.callback()
            if self.linked_text:
                self.linked_text.set_text(str(int(self.value)))
    ...
    def set_value(self, value):
        if self.whole_numbers:
            ...
            self.draw_w = max(
                0, min(ceiled_value / self.max_value * self.w, self.w))
...
class Button(UIElement):
    def draw(self, surface=None):
        ...
        w, h = self.rect.size
        if self.center_text:
            rect = ((w - self.text.get_width()) / 2,
                    (h - self.text.get_height()) / 2)
    ...
    def __init__(self, x, y, w, h, text, color, callback, args,
                 center_text=False, font_size=86, offset=None):
...
class ScrollView(UIElement):
    ...
    def update_surface(self, rel):
        ...
        if rel < 0 and self.grid.ui_manager.sprites(
        )[-1].rect.bottom < self.rect.h:
            self.y -= rel
            rel = self.rect.h - \
                self.grid.ui_manager.sprites(
                )[-1].rect.bottom - self.grid.h_gap

        elif rel > 0 and self.grid.ui_manager.sprites()[0].rect.top > 0:
            self.y = 0
            rel = -(self.grid.ui_manager.sprites()
                    [0].rect.top) + self.grid.h_gap
...
class InputField(UIElement):
    def __init__(self, x, y, w, h, callback_function, icon,
                 text_size=70, text_type=str, char_limit=0, offset=None):
        super().__init__(x, y)
        self.rect = pygame.Rect(x, y, w, h)
        self.elements = []
    ...
    def remove_letter(self):
        letter = self.text[self.cursor -
                           1] if self.cursor < len(self.text) else ""
...
class DropDown(UIElement):
    def __init__(self, x, y, w, h, choices,
                 selected_color, menu_color, callback):
        super().__init__(x, y)
        self.case_height = h
        h *= len(choices) + 1
        ...
        self.choice_text = self.font.render(
            self.selected_choice, 1, (255, 255, 255))
        self.image.blit(self.choice_text, (0, 0))
        pygame.draw.rect(self.image, (0, 0, 0, 0),
                         (0, self.case_height, self.rect.w, self.rect.h))

    def set_choice(self, btn, choice):
        self.selected_choice = choice
        pygame.draw.rect(self.image, self.selected_color,
    ...
    def update(self, mouse_position, mouse_pressed, mouse_rel, events):
        ...
        for event in events:
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                ...
                if not self.opened:
                    pygame.draw.rect(
                        self.image, (0, 0, 0, 0), (0, self.case_height, self.rect.w, self.rect.h))
                if self.opened:
                    self.ui_manager.draw(self.image)
...
class LevelButton(Button):
    def __init__(self, x, y, w, h, map_name, map_author, color,
                 callback, args, center_text=False, font_size=46, offset=None):
        super().__init__(x, y, w, h, " ", color, callback,
                         args, font_size=font_size, offset=offset)
...
class Grid(UIElement):
    ...
    def add(self, ui_class, ui_params):
        if (self.line != 0 or self.row != 0) and self.row * \
                (self.w_size + self.w_gap) + self.w_size > self.rect.w:
            self.line += 1
            self.row = 0
