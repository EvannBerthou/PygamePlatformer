...
class Editor:
    ...
    def save_to_file(self, btn, args):
        ...
        player_size = 32 if self.player_size.text == "" else int(
            self.player_size.text)
        if name and author:
            self.map_path = os.path.join('custom_maps', name)
            print(save_to_file(self.rects.sprites(), self.map_path,
                               name, author, self.dialogues, player_size))
        else:
    ...
    def open_map(self, btn, args):
        ...
        filename = filedialog.askopenfilename(
            initialdir='.', title='Select map to open')
    ...
    def __init__(self, game):
        ...
        self.wall_button = UI.Button(
            2, 200, 200, 60, "Wall", (255, 0, 0), self.change_object, Wall)
        self.door_button = UI.Button(
            2, 280, 200, 60, "Door", (150, 150, 150), self.change_object, Door)
        self.spawn_button = UI.Button(
            2, 360, 200, 60, "Spawn", (150, 150, 150), self.change_object, SpawnPoint)
        self.plate_button = UI.Button(
            2, 440, 200, 60, "Plate", (150, 150, 150), self.change_object, Plate)
        self.goal_button = UI.Button(
            2, 520, 200, 60, "Goal", (150, 150, 150), self.change_object, EndGoal)
        self.option_button = UI.Button(
            5,
            DESIGN_H - 110,
            100,
            100,
            ...
            [],
            center_text=True)
        self.x_grid_text = UI.Text(
            740, DESIGN_H - 110, self.grid_x, 72, (255, 255, 255))
        self.y_grid_text = UI.Text(
            740, DESIGN_H - 50, self.grid_y, 72, (255, 255, 255))
        self.x_grid_slider = UI.Slider(
            130,
            DESIGN_H - 110,
            600,
            40,
        ...
        self.map_name_text = UI.Text(
            DESIGN_W / 2 - 500, DESIGN_H / 2 - 450, "Map name", 72, (255, 255, 255))
        self.map_name = UI.InputField(
            DESIGN_W / 2 - 500, DESIGN_H / 2 - 400, 800, 60, None, None, 100)
        self.map_author_text = UI.Text(
            DESIGN_W / 2 - 500, DESIGN_H / 2 - 300, "Map author", 72, (255, 255, 255))
        self.map_author = UI.InputField(
            DESIGN_W / 2 - 500, DESIGN_H / 2 - 250, 800, 60, None, None, 100)
        self.player_size_text = UI.Text(
            DESIGN_W / 2 - 500, DESIGN_H / 2 - 150, "Player size", 72, (255, 255, 255))
        self.player_size = UI.InputField(
            DESIGN_W /
            2 -
            440 +
    ...
    def update(self, mouse_position, mouse_pressed, mouse_rel, events):
        ...
        # UPDATE
        self.clicked_on_ui = self.ui_to_draw.update(
            mouse_position, mouse_pressed, 0, events)
        ...
        if self.selected_rect != - \
                1 and isinstance(rect, Plate) and rect.linked_to_id != -1:
        ...
        if self.rect_started:
            ...
            DragRect(*self.rect_start, *size, (0, 255, 0)
                     ).draw(self.camera, self.custom_blitting)

        if self.property_panel:
            if self.property_panel.linking:
                self.camera.draw_line(self.custom_blitting, (0, 255, 0),
                                      (self.rects.sprites()[
                                       self.selected_rect].org_rect.center[:2]),
                                      self.camera.screen_to_world(mouse_position), 10)

        self.camera.draw_rect(self.custom_blitting,
                              (255, 255, 255), (0, 0, DESIGN_W, DESIGN_H), 3)
        self.custom_blitting.blit(self.camera.scale_to_zoom(
            self.grid), self.camera.get_offset(self.grid.get_rect()))
    ...
    def draw_arrows(self, rect):
        self.vertical_arrow = pygame.Rect(
            rect.center[0] + 40, rect.center[1], 140, 40)
        self.horizontal_arrow = pygame.Rect(
            rect.center[0], rect.center[1], 40, 140)
        pygame.draw.rect(self.custom_blitting,
                         (0, 0, 255), self.vertical_arrow)
        pygame.draw.rect(self.custom_blitting, (0, 255, 0),
